...
def apply_filters(all_parsed_rows, parse_errors):
    ...
    Year filtering and org selection go in the sidebar.
    Duplicate detection warnings go in the main area.
    ...
    # Multi-org selector and year filtering go in the sidebar
    with st.sidebar:
        if len(ein_list) > 1:
            st.markdown(
                '<div style="font-size:0.65rem;font-weight:600;color:#94A3B8;'
                'letter-spacing:0.05em;text-transform:uppercase;margin:8px 0 4px;">'
                'Organization</div>',
                unsafe_allow_html=True,
            )
            org_options = {f"{smart_title(info['name'])[:30]} ({ein})": ein for ein, info in ein_map.items()}
